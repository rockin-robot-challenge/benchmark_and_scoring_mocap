...
if (__name__ == "__main__"):
    ...
    while not rospy.is_shutdown():
		...
		counter=0
		waypoint_name, waypoint_x, waypoint_y = np.loadtxt('/home/robocup/ros/src/benchmark_and_scoring_mocap/tools/erl_waypoints.csv', skiprows=1, dtype='str', unpack=True, delimiter=',')
		waypoint_x = waypoint_x.astype('float')
		waypoint_y = waypoint_y.astype('float')

		for name, x, y in zip(waypoint_name, waypoint_x, waypoint_y):
			...
			single_marker.pose.position.x = single_marker_text.pose.position.x = x
			single_marker.pose.position.y = single_marker_text.pose.position.y = y
			single_marker.pose.position.z = single_marker_text.pose.position.z = 0.0

			(qx,qy,qz,qw) = tf.transformations.quaternion_from_euler(0.0, 0.0, 0.0)
